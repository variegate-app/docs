...
- Не закрывать/не дождаться закрытия `c`: consumer может ждать значения навсегда.
- Завершать `done`/отмену, но продолжать ожидать все результаты (получаются блокировки/гонки логики).
- Смешивать ответственность за закрытие каналов между горутинами.

## Практический чеклист
- Ограничение параллелизма задано фиксированным числом goroutines (`numDigesters`).
- Есть корректная стратегия отмены: `done` останавливает отправку в каналы.
- Выходной канал закрывается ровно один раз (после `wg.Wait()`).
- Ошибки обрабатываются предсказуемо: определите “кто” и “когда” сообщает об ошибке.

## Как адаптировать под кейс
- Для более сложных задач добавьте отдельный pipeline шаг: чтение -> обработка -> агрегация.
- Размер буфера каналов подберите под ожидаемую задержку чтения/вычисления.

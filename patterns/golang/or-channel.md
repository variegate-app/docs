...
- Передавать в `or-channel` каналы, жизненный цикл которых вам не подконтролен (может быть раннее закрытие).
- Создавать `or-channel` внутри горячего пути без переиспользования: растёт число горутин.
- Забывать, что “закрытие” сигнализирует остановку, и не проектировать нижние уровни как отменяемые.

## Практический чеклист
- В `or-channel` нет утечек: рекурсивный `or` завершается при закрытии одного из входных `done`.
- Внутренние горутины не пытаются отправлять в закрытый канал.
- Внешний код обеспечивает корректное закрытие входных `done` каналов.

## Как адаптировать под кейс
- Если отмена источников может происходить “асинхронно” — `or-channel` упрощает модель остановки.
- Для больших наборов входных каналов проверьте число аллокаций/горутин.

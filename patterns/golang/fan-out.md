...
- Не ограничивать число активных горутин: фан-аут превращается в неконтролируемый параллелизм.
- Не закрывать канал задач: часть worker'ов продолжит ждать “следующую работу”.
- Проглатывать сигналы завершения и не делать корректный drain/согласованное shutdown.

## Практический чеклист
- В fan-out есть координация закрытия входов и ожидание `wg`/завершения задач.
- Если tasks генерируются быстрее обработки — используется буфер + лимиты.
- При необходимости вводится semaphore/ограничение конкуренции (fan-out bounded).
- Все send/recv ветки учитывают завершение и корректное закрытие каналов.

## Как адаптировать под кейс
- При пакетной обработке используйте ограниченный вариант (bounded) — так проще управлять нагрузкой.
- Если нужен round-robin/распределение равномернее — проверьте нужные свойства очереди/буфера.

...
- Использовать “pooling” через канал без корректного закрытия входа: пул не завершится.
- Отправлять больше задач сигналами, чем способен принять пул (backpressure отсутствует).
- Не понимать, что “сигналы” по небуферизованному каналу синхронизируют (возможна лишняя блокировка).

## Практический чеклист
- Канал/механизм сигналов имеет понятную семантику: где блокируем, где нет.
- Менеджер закрывает канал после отправки всех задач.
- Worker'ы корректно завершаются по закрытию канала.
- Если нужен ограниченный параллелизм — `Pooling` дополняется `worker pool/bounded parallelism`.

## Как адаптировать под кейс
- Для “готовности воркера” pooling удобно, когда задачи одинаковые по обработке.
- Для разных классов задач выделяйте отдельные очереди/каналы.

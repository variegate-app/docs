...
- Выбирать “drop” без понимания последствий: можно незаметно потерять критичные события/запросы.
- Ставить drop на неправильном уровне (теряется смысл backpressure и SLO).
- Не закрывать канал после shutdown: worker будет ждать следующую работу.

## Практический чеклист
- Понятно, что именно выбрасывается (какие классы сообщений) и по какому правилу.
- Есть ограничение по времени/объёму (capacity buffer как “источник правды”).
- shutdown корректный: закрытие входов и завершение worker'ов.
- Поведение документировано: что происходит при переполнении (drop vs block vs retry).

## Как адаптировать под кейс
- Для метрик/телеметрии drop часто приемлем (лучше свежие данные, чем все).
- Для транзакционных операций drop обычно опасен — используйте отдельную очередь или retry с лимитами.

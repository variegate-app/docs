...
- Таймаут задан, но рабочая горутина не прекращает работу при `ctx.Done()` (получается утечка/“висящая” goroutine).
- Использовать буфер не по задаче: отправителю некуда слать результат, и он блокируется.
- Создавать таймаутный/отменяемый контекст без `defer cancel()` (утрачиваются связанные ресурсы).

## Практический чеклист
- В рабочих горутинах есть ветка `case <-ctx.Done(): ... return`.
- Каналы сигналов/результатов либо буферизованы осмысленно, либо гарантирован consumer.
- Внешние операции принимают `ctx` (DB/network/RPC), а не игнорируют его.
- `cancel()` вызывается когда задача завершилась (успешно или ошибкой/таймаутом).

## Как адаптировать под кейс
- Сформулируйте, что значит “результат больше не нужен” (остановить вычисления, отменить HTTP/RPC, прекратить чтение из очереди).
- Определите, нужно ли возвращать ошибку `ctx.Err()` наверх или маппить в доменную ошибку.

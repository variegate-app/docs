...
### Пример реализации на Go (Plugin: динамическая привязка)

Ниже пример “plugin factory”: конфигурация задаёт имя реализации, а фабрика создаёт нужный объект в runtime.
...
// Interface, общий контракт поведения.
type KeyGenerator interface {
	Generate() string
}

// Реализации.
type CounterGenerator struct{ n int }

func (g *CounterGenerator) Generate() string {
	g.n++
	return fmt.Sprintf("counter-%d", g.n)
}

type DBSequenceGenerator struct{}

func (g *DBSequenceGenerator) Generate() string {
	return "db-seq-100500"
}

// Фабрика (registry) по имени.
type Factory struct {
	constructors map[string]func() KeyGenerator
}

func NewFactory() Factory {
	return Factory{
		constructors: map[string]func() KeyGenerator{
			"counter": func() KeyGenerator { return &CounterGenerator{} },
			"dbseq":   func() KeyGenerator { return &DBSequenceGenerator{} },
		},
	}
}

func (f Factory) Create(name string) (KeyGenerator, error) {
...
func main() {
	// допустим, прочитали из файла конфигурации
	cfg := "counter"

	factory := NewFactory()
	gen, _ := factory.Create(cfg)
